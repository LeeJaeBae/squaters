...
export const dbCreate = (req, res) => {
    const createSql = {
       ...
       calendar_succeed     TINYINT(1)      default FALSE,
       ...
       last_exercise_data   INT(3)          default NULL,
       ...
       exercise_succeed   TINYINT(1)      default FALSE,
    ...
    db.query("SELECT * FROM user WHERE user_id=test")
        ...
        .then(
            ...
            (err) => {
                ...
                return db.query("INSERT INTO user_status (user_no) VALUES (?)", [user_no]);
            }
        )
        .then(() => {
...
export const calendarCreate = (req, res) => {
    let createLevel = req.query.level;
            ...
            (async function () {
                ...
                for (let value of calendarNo) {
                    for (let j = 0; j < 5; j++) {
                        ...
                        await db
                            .query(
                                "INSERT INTO exercise (calendar_no, exercise_data, exercise_type, exercise_succeed) VALUES (?, ?, ?, ?)",
                                [value, ...temp, false]
                            )
                            .then(
                                () => {
...
export const exerciseDelete = (req, res) => {
    ...
    db.query("SELECT calendar_no FROM calendar WHERE user_no=? AND calendar_id=?", [
        ...
        .then((row) => {
            return db.query("SELECT exercise_no FROM exercise WHERE calendar_no=?", [
                row[0].calendar_no,
            ]);
        })
        .then((rows) => {
            ...
            } else if (rutine.length === exerciseId) {
                deleteRow.push(rutine[rutine.length - 1]);
                if (rutine.length > 1) {
                    deleteRow.push(rutine[rutine.length - 2]);
                }
            } else {
                deleteRow.push(rutine[exerciseId - 1], rutine[exerciseId]);
            }
            for (let i = 0; i < deleteRow.length; i++) {
                db.query("DELETE FROM exercise WHERE exercise_no=?", [deleteRow[i]]).catch(
                    (err) => {
                        db.end();
                        return res.end();
                    }
...
export const calendarInquiry = (req, res) => {
    ...
    db.query("SELECT last_date, user_level FROM user_status WHERE user_no=?", [user_no])
        .then((rows) => {
            if (rows[0].last_date !== null) {
                let createdDate =
                    ...
                    (1000 * 3600 * 24);
                if (createdDate === 30) {
                    lastLevel.level = rows[0].user_level;
                } else {
                    lastLevel.level = 0;
                    db.query(
        ...
        .then(() => {
            res.JSON({ level: lastLevel, calendarData: calenadarData });
        })
        .catch((err) => {
            db.end();
            return res.end();
...
export const exerciseGet = (req, res) => {
    ...
    db.query(
        "SELECT A.exercise_data, A.exercise_type, A.exercise_succeeded FROM exercise A " +
            "LEFT JOIN user_status B ON A.calendar_no = B.calendar_no " +
            "WHERE user_no=? AND A.exercise_succeeded=0 LIMIT 1",
        [user_no]
    )
        .then((rows) => {
...
export const exerciseSet = (req, res) => {
    ...
    db.query(
        "SELECT A.exercise_no FROM exercise A " +
            "LEFT JOIN user_status B ON A.calendar_no = B.calendar_no " +
            "WHERE user_no=1 AND A.exercise_succeeded=0 LIMIT 1",
        [user_no]
    )
        .then((rows) => {
