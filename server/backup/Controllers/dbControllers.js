...
export const dbCreate = (req, res) => {
  ...
  db.query(
    `INSERT INTO plan(cl_data,p_data,user_no) VALUES(0, 15,${user_no});`,
    () => {}
  );
  db.query(
  ...
  for (let i = 0; i < 45; i++) {
    ...
    db.query(
      `INSERT INTO default_level(lv_data, user_no) VALUES(${
        i + 1
      }, ${user_no});`,
      () => {}
    );
    ...
    db.query(
      `SELECT lv_id FROM default_level WHERE user_no = ${user_no} AND lv_data = ${
        i + 1
      };`,
      (err, levleId) => {
        db.query(
...
export const selectExercise = (req, res) => {
  ...
  db.query(
    `SELECT * FROM caledar WHERE user_no = ${user_no}`,
    (err, calendar) => {
...
export const selectLevel = (req, res) => {
  db.query(
    `SELECT * FROM calender WHERE user_no = ${user_no}`,
    (err, calendar) => {
      // 계획이 있으면 캘린더 페이지 이동
